...
module Slim::Helpers
  ...
  def get_file_path(file)
    ...
    path = extdir + '/../../../application/dist' + file
    path
  end

  def base64EncodePNG(path)
    base64 = 'data:image/png;base64,'
    File.open(path, 'r') do|image_file|
      base64 += Base64.encode64(image_file.read)
      base64.gsub!(/\n/, '')
    end
    base64
  end
  ...
  def include_css(file)
    ...
    if (attr? 'application-url')
      ...
      result = '<style>'
      result += ::IO.read(::File.join(path)).chomp
      result += '</style>'
    end
  ...
  def include_src_image(file)
    result = ''
    if (attr? 'application-url')
      result = file
    else
      path = get_file_path file
      result = base64EncodePNG path
    end

    result
  end

  def include_background_img(file)
    result = ''
    if (attr? 'application-url')
      result = ''
    else
